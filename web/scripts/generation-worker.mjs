...
const token = process.env.VENLINKS_PRO_MAINTENANCE_TOKEN?.trim() || "";
const origin = resolveGenerationWorkerOrigin();
const workerId = (process.env.VENLINKS_PRO_GENERATION_WORKER_ID?.trim() || `generation-worker:${hostname()}:${process.pid}:${randomUUID()}`).slice(0, 150);
const idleDelayMs = boundedNumber(process.env.VENLINKS_PRO_GENERATION_WORKER_INTERVAL_MS, 2_000, 500, 30_000);
const lanes = boundedNumber(process.env.VENLINKS_PRO_GENERATION_WORKER_LANES, 2, 1, 8);
const heartbeatIntervalMs = boundedNumber(process.env.VENLINKS_PRO_GENERATION_WORKER_HEARTBEAT_MS, 15_000, 5_000, 60_000);
let stopping = false;
let heartbeatPending = false;

if (token.length < 32) throw new Error("VENLINKS_PRO_MAINTENANCE_TOKEN must contain at least 32 characters");
        ...
        try {
            const response = await fetch(`${origin}/api/maintenance/generation-tasks/run`, {
                ...
                headers: {
                    ...
                    "x-venlinks-pro-worker-id": laneId,
                },
...
async function sendHeartbeat() {
    ...
    try {
        const response = await fetch(`${origin}/api/maintenance/generation-tasks/heartbeat`, {
            ...
            headers: {
                authorization: `Bearer ${token}`,
                "x-venlinks-pro-worker-id": workerId,
            },

...
function stableMediaUrl(value: unknown) {
    ...
    if (!source) return "";
    try {
        const path = new URL(source, "http://venlinks.local").pathname;
        return path.startsWith("/api/reference-assets/") || path.startsWith("/api/generation-log-assets/") ? `${path}${new URL(source, "http://venlinks.local").search}` : "";
    } catch {
        return "";
    }
}

...
- [体验] `/create`、短剧项目 Agent、Canvas Agent 与图片/视频工作台统一展示可验证执行轨迹，按需求理解、能力检查、任务执行、结果复核和交付阶段保留进度摘要；并行任务显示完成数、失败数、重试次数与耗时，断线恢复、暂停及终态均可见，且不暴露平台提示词、模型选择理由或隐藏推理。
- [修复] 文本、图片、视频和音频在上游提交异常且无法取得可靠任务 ID 时立即进入失败终态、执行原有退款并向用户提示“生成失败，请联系管理员”，不再停留在生成中等待后台人工同步结果。
- [界面] 公开首页第一屏升级为现代深空生成引擎视觉，以品牌核心、动态轨道和商业视觉/智能短剧节点强化右侧视觉主场；首页及登录入口移除美颜业务表达，改为商业视觉、智能短剧、自由画布与 Agent 编排。
- [品牌] 全仓品牌统一为 VenLinks，页面与文档文案、部署环境变量、协议头、Cookie/存储键、数据库前缀、包名、组件名、资源文件名及文档截图同步迁移并移除旧品牌残留。
- [部署] 新增仅由 `dev` 分支 push 创建的 GitLab CI 手动部署作业，复用服务器 Docker 构建缓存并在更新 App 与 Worker 后等待容器健康。
- [部署] Docker 宿主机端口改为可通过 `VENLINKS_HOST_PORT` 配置并默认使用 `3002`，容器内部服务继续固定使用 `3000`，避免与服务器现有应用冲突。
...
- [修复] New API 下的 Happy Horse 文生视频按模型族发送专用 JSON 参数；图生视频遵循 `/videos` 的 multipart 契约上传单个 `input_reference` 文件，并使用原生 Fetch 保留 multipart 边界和 `model` 表单项，避免网关误判缺少参考图或模型名。
- [修复] 短剧内容提取与视觉方案改为持久化异步文本任务，页面通过短轮询恢复结果，不再被反向代理 60 秒超时截断；长视觉方案按 8 个目标镜头分批生成并携带相邻镜头上下文，按原顺序合并，避免一次返回数十个镜头触发 8192 tokens 截断；结构缺失或模型输出截断会退款并切换备用渠道。
...
- [品牌/文档] 站点 Logo、ICO、favicon、manifest、登录页、管理后台和助手头像统一使用VenLinks 鲸鱼品牌资源；README、部署说明、API 文档和数据库文档与当前发布保持一致。
...
- [文档] README 首屏改为公开首页截图；补充完整用户端、管理后台、平台能力与部署功能清单，以及仓库目录、关键文件职责、参与贡献指南、VenLinks 开源 QQ 交流群、赞助二维码和项目致谢；公开文案统一使用“画布”和“VenLinks”，并移除旧 QQ、爱发电及未确认的 Telegram/X/Instagram 默认入口。

...
- 所有默认站点品牌 Logo、ICO、favicon、manifest/metadata 图标、登录/用户端/后台品牌标识和助手头像必须使用项目内置的 VenLinks 蓝紫色鲸鱼 Logo，并统一从后台站点 Logo/图标配置或内置 `/logo.png`、`/icon.png` 派生；禁止使用三角形或旧无限符号作为品牌 Logo、默认占位或回退图标。播放、告警等功能性三角图标不属于品牌标识，仍按语义使用。
...
- New API 的 `/videos` 图生视频请求必须按渠道声明的 multipart 契约上传参考文件；不得仅根据最终模型供应商的直连接口文档，把 `input_reference` 改成 JSON 图片字段。模型专属字段只在当前渠道协议、路径和 Content-Type 均有代码或真实请求证据时启用。

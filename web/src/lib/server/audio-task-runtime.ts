...
async function completeAudioTask(task: AudioTask, url: string, mimeType: string) {
    ...
    if (!completed) return getAudioTask(task.id);
...
function readBilling(headers: Headers) {
    const raw = headers.get("x-venlinks-pro-points-cost");
    const value = raw === null ? undefined : Number(raw);
    return { pointsCost: value !== undefined && Number.isFinite(value) && value >= 0 ? value : undefined, pointsRecordId: headers.get("x-venlinks-pro-points-record-id") || undefined };
}

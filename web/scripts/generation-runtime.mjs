...
export function generationRuntimeEnvironment({ environment = process.env, allowEphemeralToken = false } = {}) {
    ...
    const configuredToken = source.VENLINKS_MAINTENANCE_TOKEN?.trim() || "";
    if (configuredToken.length < MIN_TOKEN_LENGTH && !allowEphemeralToken) {
        throw new Error("VENLINKS_MAINTENANCE_TOKEN must contain at least 32 characters");
    }
    ...
    return {
        environment: {
            ...source,
            VENLINKS_MAINTENANCE_TOKEN: configuredToken.length >= MIN_TOKEN_LENGTH ? configuredToken : randomBytes(32).toString("hex"),
            VENLINKS_WORKER_API_ORIGIN: resolveGenerationWorkerOrigin({ environment: source, fallbackOrigin: `http://127.0.0.1:${port}` }),
        },
...
export function resolveGenerationWorkerOrigin({ environment = process.env, fallbackOrigin = "http://127.0.0.1:3000" } = {}) {
    const raw = environment.VENLINKS_WORKER_API_ORIGIN?.trim() || environment.VENLINKS_INTERNAL_ORIGIN?.trim() || environment.NEXT_PUBLIC_SITE_URL?.trim() || fallbackOrigin;

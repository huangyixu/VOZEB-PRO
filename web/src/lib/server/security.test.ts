...
describe("checkRateLimit", () => {
    ...
    it("only trusts the configured number of proxy hops", () => {
        const previous = process.env.VENLINKS_TRUSTED_PROXY_HOPS;
        delete process.env.VENLINKS_TRUSTED_PROXY_HOPS;
        expect(getClientIp(new Request("http://localhost", { headers: { "x-forwarded-for": "198.51.100.10" } }))).toBe("unknown");

        process.env.VENLINKS_TRUSTED_PROXY_HOPS = "2";
        ...
        if (previous === undefined) delete process.env.VENLINKS_TRUSTED_PROXY_HOPS;
        else process.env.VENLINKS_TRUSTED_PROXY_HOPS = previous;
    });
    ...
    it("limits generation requests across users sharing an IP", async () => {
        const previous = process.env.VENLINKS_TRUSTED_PROXY_HOPS;
        process.env.VENLINKS_TRUSTED_PROXY_HOPS = "1";
    ...
    it("allows private upstreams only through an explicit exact host allowlist", async () => {
        vi.stubEnv("VENLINKS_ALLOW_PRIVATE_UPSTREAMS", "");
        vi.stubEnv("VENLINKS_PRIVATE_UPSTREAM_HOSTS", "");
        await expect(isSafeOutboundUrl("http://127.0.0.1:4010/v1/models")).resolves.toBe(false);

        vi.stubEnv("VENLINKS_ALLOW_PRIVATE_UPSTREAMS", "1");
        await expect(isSafeOutboundUrl("http://127.0.0.1:4010/v1/models")).resolves.toBe(false);

        vi.stubEnv("VENLINKS_PRIVATE_UPSTREAM_HOSTS", "127.0.0.1, provider.internal");

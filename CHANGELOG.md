...
- [部署] Docker 宿主机端口改为可通过 `VENLINKS_PRO_HOST_PORT` 配置并默认使用 `3002`，容器内部服务继续固定使用 `3000`，避免与服务器现有应用冲突。
...
- [文档] README 首屏改为公开首页截图；补充完整用户端、管理后台、平台能力与部署功能清单，以及仓库目录、关键文件职责、参与贡献指南、VenLinks 开源 QQ 交流群、赞助二维码和项目致谢；公开文案统一使用“画布”和“VenLinks PRO”，并移除旧 QQ、爱发电及未确认的 Telegram/X/Instagram 默认入口。

...
const runtime = globalThis as typeof globalThis & { __venLinksGenerationWorkerHeartbeats?: Map<string, number> };
const fileHeartbeats = (runtime.__venLinksGenerationWorkerHeartbeats ??= new Map<string, number>());
...
export async function getGenerationWorkerHealth(now = Date.now()) {
    const staleAfterMs = normalizeGenerationWorkerStaleMs(process.env.VENLINKS_GENERATION_WORKER_STALE_MS);

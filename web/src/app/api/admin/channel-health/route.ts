...
const globalCooldownStore = globalThis as typeof globalThis & { __venLinksChannelHealthCooldowns?: Map<string, number> };
const healthCooldowns = (globalCooldownStore.__venLinksChannelHealthCooldowns ??= new Map<string, number>());
    ...
    for (const responseFormat of ["url", "b64_json"] as const) {
        ...
        if (response.ok && !isProviderBusinessError(payload)) {
            const result: HealthResult = {
                ...
                referenceRule: "图生图使用 /images/edits；VenLinks 会按 multipart、image、images、image_url、input_image 等常见字段自动兼容。",
...
async function testAudio(baseUrl: string, apiKey: string, model: string): Promise<HealthResult> {
    const response = await fetch(apiUrl(baseUrl, "/audio/speech"), {
        method: "POST",
        headers: jsonHeaders(apiKey),
        body: JSON.stringify({ model, input: "VenLinks audio health check.", voice: "alloy", response_format: "mp3" }),
        cache: "no-store",
        signal: AbortSignal.timeout(HEALTH_REQUEST_TIMEOUT_MS),
    });
...
function videoHealthConfig(baseUrl: string, model: string, path: string): Partial<HealthResult> {
    ...
    if (path === "/videos") {
        return {
            ...
            referenceRule: "参考图使用 multipart 文件上传，由 VenLinks 自动组装。",
            supportsReferenceImage: true,
...
function pointsInfo(headers: Headers) {
    const pointsCost = numericHeader(headers, "x-venlinks-points-cost");
    const pointsRemaining = numericHeader(headers, "x-venlinks-points-remaining");
    return {

...
const runtime = globalThis as typeof globalThis & { __venLinksProGenerationWorkerHeartbeats?: Map<string, number> };
const fileHeartbeats = (runtime.__venLinksProGenerationWorkerHeartbeats ??= new Map<string, number>());
...
export async function getGenerationWorkerHealth(now = Date.now()) {
    const staleAfterMs = normalizeGenerationWorkerStaleMs(process.env.VENLINKS_PRO_GENERATION_WORKER_STALE_MS);

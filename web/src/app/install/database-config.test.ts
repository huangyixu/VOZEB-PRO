...
const baseConfig = {
    ...
    host: "127.0.0.1",
    port: "5432",
    database: "venlinks_pro",
    username: "venlinks_pro",
...
describe("database deployment config", () => {
    ...
    it("uses host networking without a bundled PostgreSQL service in Baota mode", () => {
        ...
        expect(snippets.envText).toContain("@127.0.0.1:5432/venlinks_pro");
        ...
        expect(snippets.composeText).toContain(`VENLINKS_PRO_ENCRYPTION_KEY: "${baseConfig.encryptionKey}"`);
        expect(snippets.envText).toContain(`VENLINKS_PRO_MAINTENANCE_TOKEN=${baseConfig.maintenanceToken}`);
        expect(snippets.composeText.match(/VENLINKS_PRO_MAINTENANCE_TOKEN:/g)).toHaveLength(2);
        expect(snippets.composeText).toContain("generation-worker:");
        expect(snippets.composeText).toContain("VENLINKS_PRO_WORKER_API_ORIGIN: http://127.0.0.1:3000");
        expect(snippets.envText).toContain("VENLINKS_PRO_TRUSTED_PROXY_HOPS=1");
        expect(snippets.composeText).toContain('VENLINKS_PRO_TRUSTED_PROXY_HOPS: "1"');
    });
    ...
    it.each([
        ...
        expect(snippets.envText).not.toContain("VENLINKS_PRO_TRUSTED_PROXY_HOPS");
        expect(snippets.composeText).not.toContain("VENLINKS_PRO_TRUSTED_PROXY_HOPS");
        expect(snippets.composeText).toContain("generation-worker:");
        expect(snippets.composeText.match(/VENLINKS_PRO_MAINTENANCE_TOKEN:/g)).toHaveLength(2);
    });
    ...
    it("uses the Compose service name for the bundled Worker origin", () => {
        ...
        expect(snippets.composeText).toContain("VENLINKS_PRO_WORKER_API_ORIGIN: http://app:3000");
    ...
    it.each([
        ...
        expect(document.services.app.environment.VENLINKS_PRO_MAINTENANCE_TOKEN).toBe(baseConfig.maintenanceToken);
        expect(document.services["generation-worker"].environment.VENLINKS_PRO_MAINTENANCE_TOKEN).toBe(baseConfig.maintenanceToken);
    });
});

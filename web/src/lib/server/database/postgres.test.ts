...
describe("PostgreSQL schema lifecycle", () => {
    beforeEach(() => {
        delete (globalThis as Record<string, unknown>).__venLinksPostgresPool;
        delete (globalThis as Record<string, unknown>).__venLinksPostgresSchemaReady;
    ...
    it("executes schema DDL only through explicit initialization", async () => {
        ...
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS venlinks_schema_migrations");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS venlinks_generation_worker_heartbeats");
        expect(ddl).toContain("CREATE SEQUENCE IF NOT EXISTS venlinks_user_account_id_seq");
        expect(ddl).toContain("account_id bigint NOT NULL DEFAULT nextval('venlinks_user_account_id_seq')");
        expect(ddl).toContain("CREATE UNIQUE INDEX IF NOT EXISTS venlinks_users_account_id_idx ON venlinks_users (account_id)");
        expect(ddl).toContain("user_id text NOT NULL REFERENCES venlinks_users(id) ON DELETE CASCADE");
        expect(ddl).toContain("CREATE TABLE IF NOT EXISTS venlinks_account_deletion_requests");
        ...
        expect(tableNames.every((name) => name.startsWith("venlinks_"))).toBe(true);
        expect(tableNames).not.toContain("venlinks_check_ins");
        expect(ddl).toContain("DROP TABLE IF EXISTS venlinks_check_ins");
        ...
        expect(indexNames.every((name) => name.startsWith("venlinks_"))).toBe(true);
        ...
        expect(uniqueConstraintNames.every((name) => name.startsWith("venlinks_"))).toBe(true);
    });

    it("continues applying additive schema updates after the sentinel table exists", async () => {
        mocks.query.mockResolvedValueOnce({ rows: [{ table_name: "venlinks_users" }] }).mockResolvedValueOnce({ rows: [] });

        await ensurePostgresSchema();

        expect(mocks.query).toHaveBeenCalledTimes(2);
        expect(mocks.query.mock.calls[0]?.[0]).toContain("to_regclass");
        expect(mocks.query.mock.calls[1]?.[0]).toContain("CREATE TABLE IF NOT EXISTS venlinks_schema_migrations");
    });
});

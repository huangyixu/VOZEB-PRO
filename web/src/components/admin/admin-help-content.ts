...
export const adminHelpArticles: AdminHelpArticle[] = [
    {
        ...
        steps: [
            ...
            {
                title: "配置站点和模型能力",
                description: "先设置站点名称与VenLinks 鲸鱼 Logo，再配置上游渠道、逻辑模型和默认文本模型。",
    ...
    },
    {
        ...
        steps: [
            ...
            {
                title: "创建逻辑模型和绑定",
                description: "为用户侧能力设置名称、类型和启停状态，再绑定渠道、真实模型与优先级。",
                checks: ["逻辑模型类型与上游能力一致", "至少一个启用绑定可用", "基础积分键使用逻辑模型 ID"],
            },
            {
                title: "设置系统默认模型",
    ...
    },
    {
        ...
        steps: [
            {
                title: "维护站点资料和品牌",
                description: "设置站点名称、SEO、社交入口、首页展示和VenLinks 鲸鱼 Logo；浏览器图标与助手头像使用同一品牌来源。",
        ...
        checks: ["站点所有品牌入口使用VenLinks 鲸鱼 Logo", "注册、验证码和找回密码流程与 SMTP 状态一致", "注销和升级操作都有可追踪记录"],

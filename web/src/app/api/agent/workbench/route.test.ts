...
describe("workbench agent model routing", () => {
    ...
    it("sends the logical billing model, stable request key, and cancellation signal", async () => {
        ...
        expect(response.status).toBe(200);
        expect(headers.get("x-venlinks-logical-model")).toBe("planner");
        expect(headers.get("x-venlinks-points-idempotency-key")).toMatch(/^workbench-plan:[a-f0-9]{32}:chat-json$/);
    ...
    it("uses distinct idempotency keys for different planner bindings on the same channel", async () => {
        ...
        const calls = mocks.fetchInternalApi.mock.calls.map(([, init]) => ({
            ...
            key: new Headers(init?.headers).get("x-venlinks-points-idempotency-key"),
        }));
    ...
    it("rejects prose chat output instead of guessing generation parameters locally", async () => {
        mocks.fetchInternalApi.mockResolvedValueOnce(
            Response.json({ choices: [{ message: { content: "Use a warm composition and generate three images." } }] }, { headers: { "x-venlinks-points-cost": "1", "x-venlinks-points-record-id": "points-workbench-1" } }),
        );
    ...
    it("refunds the free-text quota when a zero-cost planner response is invalid", async () => {
        mocks.fetchInternalApi.mockResolvedValueOnce(Response.json({ choices: [{ message: { content: "not structured" } }] }, { headers: { "x-venlinks-points-cost": "0", "x-venlinks-points-record-id": "points-workbench-free" } }));
    ...
    it("refunds a valid planner call when conversation persistence fails", async () => {
        mocks.fetchInternalApi.mockResolvedValueOnce(
            Response.json(
                ...
                { headers: { "x-venlinks-points-cost": "2", "x-venlinks-points-record-id": "points-save-failed" } },
            ),
        );

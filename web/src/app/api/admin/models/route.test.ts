...
describe("admin models route", () => {
    beforeEach(() => {
        ...
        (globalThis as typeof globalThis & { __venLinksModelFetchCooldowns?: Map<string, number> }).__venLinksModelFetchCooldowns?.clear();
    });
    ...
    it("rejects an encrypted storage value before calling the provider", async () => {
        savedChannel.apiKey = "venlinks-secret:v1:iv.tag.payload";
        const fetchMock = vi.fn();
        vi.stubGlobal("fetch", fetchMock);

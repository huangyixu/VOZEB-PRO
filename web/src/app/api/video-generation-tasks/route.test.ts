...
describe("video generation candidate failover", () => {
    ...
    it("forwards the authenticated maintenance worker identity to the internal system proxy", async () => {
        ...
        const response = await POST(
            new Request("http://localhost/api/video-generation-tasks", {
                ...
                headers: {
                    ...
                    "x-venlinks-worker-user-id": "user",
                },
        ...
        expect(headers.get("x-venlinks-worker-user-id")).toBe("user");
    ...
    it("uses separate text-to-video and image-to-video paths with trusted billing headers", async () => {
        ...
        expect(textHeaders.get("x-venlinks-logical-model")).toBe("video");
        expect(textHeaders.get("x-venlinks-upstream-model")).toBe("video-one");
        expect(textHeaders.get("x-venlinks-points-idempotency-key")).toBe("video-request:video-text");
        expect(imageHeaders.get("x-venlinks-points-idempotency-key")).toBe("video-request:video-image");
    });

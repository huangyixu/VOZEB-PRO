...
describe("video generation candidate failover", () => {
    ...
    it("forwards the authenticated maintenance worker identity to the internal system proxy", async () => {
        ...
        const response = await POST(
            new Request("http://localhost/api/video-generation-tasks", {
                ...
                headers: {
                    ...
                    "x-venlinks-pro-worker-user-id": "user",
                },
        ...
        expect(headers.get("x-venlinks-pro-worker-user-id")).toBe("user");
    ...
    it("uses separate text-to-video and image-to-video paths with trusted billing headers", async () => {
        ...
        expect(textHeaders.get("x-venlinks-pro-logical-model")).toBe("video");
        expect(textHeaders.get("x-venlinks-pro-upstream-model")).toBe("video-one");
        expect(textHeaders.get("x-venlinks-pro-points-idempotency-key")).toBe("video-request:video-text");
        expect(imageHeaders.get("x-venlinks-pro-points-idempotency-key")).toBe("video-request:video-image");
    });
    ...
    it("uses the Happy Horse request contract for New API text-to-video models", async () => {
        mocks.getAuthSettings.mockResolvedValue(newApiVideoSettings("newapi", "happyhorse-1.1-t2v"));
        mocks.fetchInternalApi.mockResolvedValue(json({ id: "upstream-happyhorse", status: "queued" }));
...
function newApiVideoSettings(protocol: "newapi" | "openai" = "newapi", upstreamModel = "video-one") {
    return {
        ...settings,
        systemChannels: [
            {
        ...
        logicalModels: [{ ...settings.logicalModels[0], bindings: [{ ...settings.logicalModels[0].bindings[0], upstreamModel }] }],
    };
}

...
const originalEncryptionKey = process.env.VENLINKS_ENCRYPTION_KEY;

afterEach(() => {
    if (originalEncryptionKey === undefined) delete process.env.VENLINKS_ENCRYPTION_KEY;
    else process.env.VENLINKS_ENCRYPTION_KEY = originalEncryptionKey;
});
...
describe("PostgreSQL auth read paths", () => {
    ...
    it("decrypts system channel API keys on the settings fast path", async () => {
        process.env.VENLINKS_ENCRYPTION_KEY = "31".repeat(32);
        ...
        expect(settings.systemChannels[0].apiKey).not.toContain("venlinks-secret:v1:");
    });

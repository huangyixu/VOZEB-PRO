ARG VENLINKS_PRO_NODE_IMAGE=node:22-bookworm-slim
ARG VENLINKS_PRO_NPM_REGISTRY=https://registry.npmjs.org
ARG VENLINKS_PRO_DEBIAN_MIRROR=http://deb.debian.org/debian
ARG VENLINKS_PRO_DEBIAN_SECURITY_MIRROR=http://deb.debian.org/debian-security

FROM ${VENLINKS_PRO_NODE_IMAGE} AS web-build

WORKDIR /app/web
ARG BUILD_NODE_OPTIONS=--max-old-space-size=1536
ARG NEXT_BUILD_CPUS=1
ARG PNPM_VERSION=11.7.0
ARG VENLINKS_PRO_NPM_REGISTRY
...
RUN npm install --global pnpm@${PNPM_VERSION} --registry=${VENLINKS_PRO_NPM_REGISTRY} \
    && pnpm config set registry ${VENLINKS_PRO_NPM_REGISTRY}

COPY web/package.json web/pnpm-lock.yaml web/pnpm-workspace.yaml ./
RUN --mount=type=cache,target=/pnpm/store pnpm install --frozen-lockfile --store-dir=/pnpm/store --registry=${VENLINKS_PRO_NPM_REGISTRY}
...
FROM ${VENLINKS_PRO_NODE_IMAGE}

WORKDIR /app
ARG VENLINKS_PRO_DEBIAN_MIRROR
ARG VENLINKS_PRO_DEBIAN_SECURITY_MIRROR
ENV NODE_ENV=production
ENV NEXT_TELEMETRY_DISABLED=1
ENV HOSTNAME=0.0.0.0
ENV PORT=3000
ENV VENLINKS_PRO_DATA_DIR=/app/web/.data
ENV VENLINKS_PRO_INTERNAL_ORIGIN=http://127.0.0.1:3000
...
RUN set -eux; \
    find /etc/apt -type f \( -name '*.list' -o -name '*.sources' \) \
        -exec sed -i \
        -e "s|http://deb.debian.org/debian-security|${VENLINKS_PRO_DEBIAN_SECURITY_MIRROR}|g" \
        -e "s|http://deb.debian.org/debian|${VENLINKS_PRO_DEBIAN_MIRROR}|g" {} +; \
    apt-get update; \

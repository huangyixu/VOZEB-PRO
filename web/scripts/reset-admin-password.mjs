...
const dataDir = path.resolve(args.dataDir || process.env.VENLINKS_PRO_DATA_DIR || path.join(webRoot, ".data"));
...
function printHelp() {
    console.log(`VenLinks PRO 管理员密码重置
...
可选参数：
  ...
  --data-dir <目录>       指定数据目录，默认使用 VENLINKS_PRO_DATA_DIR 或 web/.data

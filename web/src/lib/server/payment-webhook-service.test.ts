...
describe("payment webhook processing", () => {
    beforeEach(() => {
        ...
        mocks.getPaymentConfig.mockResolvedValue({
            ...
            valuesByEnvName: { VENLINKS_PRO_PAYPLY_WEBHOOK_SECRET: webhookSecret },
    ...
    it("records and rejects an invalid signature before claiming the event", async () => {
        const headers = new Headers({ "x-venlinks-pro-signature": "invalid" });
...
function signedHeaders(body: string) {
    return new Headers({ "x-venlinks-pro-signature": createHmac("sha256", webhookSecret).update(body).digest("hex") });
}

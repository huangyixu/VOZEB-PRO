...
describe("admin backup API", () => {
    ...
    it("uses and sanitizes the server backup filename", () => {
        expect(adminBackupFileName('attachment; filename="venlinks:backup.json"')).toBe("venlinks-backup.json");
        expect(adminBackupFileName("attachment; filename*=UTF-8''venlinks-%E5%A4%87%E4%BB%BD.json")).toBe("venlinks-备份.json");
    });

...
const customWebhookAdapter: PaymentWebhookAdapter = {
    parse(provider, rawBody, headers, paymentConfig) {
        ...
        return {
            eventId,
            eventType,
            orderId: normalizeOptionalId(readConfiguredPath(paymentConfig, payload, `${fieldPrefix}_WEBHOOK_ORDER_ID_FIELD`, ["orderId", "data.orderId", "metadata.orderId", "metadata.venLinksOrderId"])),
            orderNo: normalizeOptionalId(readConfiguredPath(paymentConfig, payload, `${fieldPrefix}_WEBHOOK_ORDER_NO_FIELD`, ["orderNo", "outTradeNo", "out_trade_no", "data.orderNo", "data.outTradeNo", "metadata.orderNo", "metadata.venLinksOrderNo"])),
...
const stripeWebhookAdapter: PaymentWebhookAdapter = {
    parse(provider, rawBody, headers, paymentConfig) {
        ...
        const orderId = normalizeOptionalId(readPath(object, "metadata.orderId") || readPath(object, "metadata.venLinksOrderId") || readPath(object, "client_reference_id"));
        const orderNo = normalizeOptionalId(readPath(object, "metadata.orderNo") || readPath(object, "metadata.venLinksOrderNo"));
...
export function verifyCustomSignature(provider: string, rawBody: string, headers: Headers, paymentConfig: PaymentRuntimeConfig) {
    ...
    const signature = normalizeSignatureHeader(headers.get(getPaymentRuntimeEnv(paymentConfig, `${providerEnvPrefix(provider)}_WEBHOOK_SIGNATURE_HEADER`) || "x-venlinks-signature") || headers.get("x-payment-signature") || headers.get("x-signature"));
    if (!signature) return false;
    const timestamp = headers.get("x-venlinks-timestamp") || headers.get("x-payment-timestamp") || "";
...
export function verifyStripeSignature(rawBody: string, headers: Headers, paymentConfig: PaymentRuntimeConfig) {
    const secret = getPaymentRuntimeValue(paymentConfig, "VENLINKS_STRIPE_WEBHOOK_SECRET", "STRIPE_WEBHOOK_SECRET");
...
export function verifyAlipaySignature(payload: Record<string, string>, paymentConfig: PaymentRuntimeConfig) {
    ...
    const appId = getPaymentRuntimeEnv(paymentConfig, "VENLINKS_ALIPAY_APP_ID");
    ...
    const content = Object.keys(payload)
        ...
        .join("&");
    return verifyRsaSha256(content, sign, loadPaymentPublicKey(paymentConfig, "VENLINKS_ALIPAY_PUBLIC_KEY", "VENLINKS_ALIPAY_PUBLIC_KEY_PATH"));
}
...
export function verifyWechatSignature(rawBody: string, headers: Headers, paymentConfig: PaymentRuntimeConfig) {
    ...
    const expectedSerial = getPaymentRuntimeEnv(paymentConfig, "VENLINKS_WECHAT_PAY_PLATFORM_CERT_SERIAL_NO");
...
export function decryptWechatResource(envelope: unknown, paymentConfig: PaymentRuntimeConfig) {
    ...
    const key = Buffer.from(requiredConfig(paymentConfig, "VENLINKS_WECHAT_PAY_API_V3_KEY"), "utf8");
...
function loadWechatPlatformPublicKey(paymentConfig: PaymentRuntimeConfig) {
    return loadPaymentPublicKey(paymentConfig, "VENLINKS_WECHAT_PAY_PLATFORM_PUBLIC_KEY", "VENLINKS_WECHAT_PAY_PLATFORM_PUBLIC_KEY_PATH", "VENLINKS_WECHAT_PAY_PLATFORM_CERTIFICATE", "VENLINKS_WECHAT_PAY_PLATFORM_CERTIFICATE_PATH");
}
...
function webhookSecret(provider: string, paymentConfig: PaymentRuntimeConfig) {
    const envKey = `VENLINKS_${provider.toUpperCase().replace(/[^A-Z0-9]/g, "_")}_WEBHOOK_SECRET`;
    return getPaymentRuntimeEnv(paymentConfig, envKey) || getPaymentRuntimeEnv(paymentConfig, "VENLINKS_PAYMENT_WEBHOOK_SECRET");
}
...
function providerEnvPrefix(provider: string) {
    return `VENLINKS_${provider.toUpperCase().replace(/[^A-Z0-9]/g, "_")}`;
}
...
export function deterministicEventId(provider: string, rawBody: string) {
    return `${provider}_${createHmac("sha256", "venlinks-webhook-event").update(rawBody).digest("hex").slice(0, 32)}`;
}

function stripeToleranceMs(paymentConfig: PaymentRuntimeConfig) {
    const seconds = Number(getPaymentRuntimeEnv(paymentConfig, "VENLINKS_STRIPE_WEBHOOK_TOLERANCE_SECONDS") || 300);
...
function wechatToleranceMs(paymentConfig: PaymentRuntimeConfig) {
    const seconds = Number(getPaymentRuntimeEnv(paymentConfig, "VENLINKS_WECHAT_PAY_WEBHOOK_TOLERANCE_SECONDS") || 300);
    return Number.isFinite(seconds) && seconds > 0 ? seconds * 1000 : 300_000;
}

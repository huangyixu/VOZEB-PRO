...
describe("admin models route", () => {
    beforeEach(() => {
        ...
        (globalThis as typeof globalThis & { __venLinksProModelFetchCooldowns?: Map<string, number> }).__venLinksProModelFetchCooldowns?.clear();
    });
    ...
    it("rejects an encrypted storage value before calling the provider", async () => {
        savedChannel.apiKey = "venlinks-pro-secret:v1:iv.tag.payload";
        const fetchMock = vi.fn();
        vi.stubGlobal("fetch", fetchMock);

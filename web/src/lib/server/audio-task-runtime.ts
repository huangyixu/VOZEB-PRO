...
function readBilling(headers: Headers) {
    const raw = headers.get("x-venlinks-points-cost");
    const value = raw === null ? undefined : Number(raw);
    return { pointsCost: value !== undefined && Number.isFinite(value) && value >= 0 ? value : undefined, pointsRecordId: headers.get("x-venlinks-points-record-id") || undefined };
}

...
export const docsComposeProfiles = [
    { file: "docs/docker-compose.yml", image: "ghcr.io/csyqlz/venlinks-docs:latest" },
    { file: "docs/docker-compose.local.yml", build: { context: "..", dockerfile: "docs/Dockerfile" }, localBuild: true },
];

const maintenanceToken = "${VENLINKS_MAINTENANCE_TOKEN:?请在 .env 中配置至少 32 位维护令牌}";
...
export function validateComposeContract(source, profile) {
    ...
    ensure(appEnvironment.VENLINKS_MAINTENANCE_TOKEN === maintenanceToken, "app 未声明强制维护令牌");
    ensure(workerEnvironment.VENLINKS_MAINTENANCE_TOKEN === maintenanceToken, "generation-worker 未声明同一强制维护令牌");
    ensure(workerEnvironment.VENLINKS_WORKER_API_ORIGIN === profile.workerOrigin, `Worker API 地址必须为 ${profile.workerOrigin}`);
    ensure(appEnvironment.VENLINKS_DATABASE_PROVIDER === "postgres", "app 必须使用 PostgreSQL provider");
    ...
    ensure(!("VENLINKS_DATABASE_PROVIDER" in workerEnvironment), "generation-worker 不应直接访问数据库 provider");
    ensure(app.volumes?.includes("venlinks-data:/app/web/.data"), "app 缺少持久数据卷挂载");
    ensure(Object.hasOwn(compose?.volumes || {}, "venlinks-data"), "缺少 venlinks-data 顶层数据卷");
    ensure(
    ...
    if (profile.embeddedPostgres) {
        ...
        ensure(Object.hasOwn(compose?.volumes || {}, "venlinks-postgres"), "内置 PostgreSQL 拓扑缺少数据库数据卷");
    } else {
        ...
        ensure(!Object.hasOwn(compose?.volumes || {}, "venlinks-postgres"), "外部数据库拓扑不得声明无用的 PostgreSQL 数据卷");
    }
...
function validateChinaBuildSources(args, violations, label) {
    if (!String(args?.VENLINKS_NODE_IMAGE || "").includes("docker.m.daocloud.io/library/node:22-bookworm-slim")) violations.push(`${label} 未使用国内 Node 镜像`);
    if (!String(args?.VENLINKS_NPM_REGISTRY || "").includes("registry.npmmirror.com")) violations.push(`${label} 未使用国内 npm 镜像`);
}

...
async function refundStripePayment(order: BillingOrderRecord, payment: PaymentTransactionRecord, options: PaymentRefundOptions, paymentConfig: PaymentRuntimeConfig): Promise<PaymentRefundResult> {
    ...
    const response = await fetch(`${stripeApiBase(paymentConfig)}/v1/refunds`, {
        ...
        headers: {
            ...
            "Idempotency-Key": `venlinks-pro-refund-${order.id}`,
        },
        body: params,
    });
...
async function refundWechatPayment(order: BillingOrderRecord, payment: PaymentTransactionRecord, options: PaymentRefundOptions, paymentConfig: PaymentRuntimeConfig): Promise<PaymentRefundResult> {
    ...
    const notifyUrl = getPaymentRuntimeEnv(paymentConfig, "VENLINKS_PRO_WECHAT_PAY_REFUND_NOTIFY_URL");
...
async function refundPayplyPayment(order: BillingOrderRecord, payment: PaymentTransactionRecord, options: PaymentRefundOptions, paymentConfig: PaymentRuntimeConfig): Promise<PaymentRefundResult> {
    const refundUrl = getPaymentRuntimeValue(paymentConfig, "VENLINKS_PRO_PAYPLY_REFUND_URL", "PAYPLY_REFUND_URL");
    ...
    const providerStatus = normalizeOptionalText(readConfiguredPath(paymentConfig, payload, "VENLINKS_PRO_PAYPLY_REFUND_STATUS_FIELD", ["refundStatus", "refund_status", "status", "data.refundStatus", "data.status", "result.status"]), 80);
    ...
    return {
        provider: "payply",
        status,
        providerRefundId: normalizeOptionalText(readConfiguredPath(paymentConfig, payload, "VENLINKS_PRO_PAYPLY_REFUND_ID_FIELD", ["refundId", "refund_id", "id", "data.refundId", "data.refund_id", "data.id", "result.id"]), 160),
        rawPayload: sanitizeJson(payload),
    };
}
...
function providerRefundRequestNo(order: BillingOrderRecord) {
    ...
    return `venlinks-pro-refund-${id || order.orderNo}`.slice(0, 64);
}
...
function buildPayplyRefundPayload(paymentConfig: PaymentRuntimeConfig, order: BillingOrderRecord, payment: PaymentTransactionRecord, options: PaymentRefundOptions) {
    ...
    const template = getPaymentRuntimeValue(paymentConfig, "VENLINKS_PRO_PAYPLY_REFUND_REQUEST_TEMPLATE", "PAYPLY_REFUND_REQUEST_TEMPLATE");
...
function payplyHeaders(config: PaymentRuntimeConfig, apiKey: string, contentType: string, idempotencyKey: string) {
    ...
    return { ...headers, ...parseExtraHeaders(config, "VENLINKS_PRO_PAYPLY_EXTRA_HEADERS", "PAYPLY_EXTRA_HEADERS"), ...parseExtraHeaders(config, "VENLINKS_PRO_PAYPLY_REFUND_EXTRA_HEADERS", "PAYPLY_REFUND_EXTRA_HEADERS") };
}
...
function normalizePayplyRefundStatus(value: string | undefined, paymentConfig: PaymentRuntimeConfig): Exclude<PaymentRefundStatus, "manual"> | undefined {
    ...
    const configured = getPaymentRuntimeEnv(paymentConfig, "VENLINKS_PRO_PAYPLY_REFUND_SUCCESS_STATUSES");
...
function payplyRefundRequestMethod(config: PaymentRuntimeConfig) {
    const method = getPaymentRuntimeValue(config, "VENLINKS_PRO_PAYPLY_REFUND_REQUEST_METHOD", "PAYPLY_REFUND_REQUEST_METHOD").toUpperCase();
    return method === "GET" ? "GET" : "POST";
}

function payplyRefundContentType(config: PaymentRuntimeConfig) {
    const contentType = getPaymentRuntimeValue(config, "VENLINKS_PRO_PAYPLY_REFUND_CONTENT_TYPE", "PAYPLY_REFUND_CONTENT_TYPE", "VENLINKS_PRO_PAYPLY_CONTENT_TYPE", "PAYPLY_CONTENT_TYPE") || "application/json";

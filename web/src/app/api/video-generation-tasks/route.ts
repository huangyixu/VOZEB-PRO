...
export async function POST(request: Request) {
    ...
    const response = await withGenerationConcurrencyLimit(user.id, "video", 30 * 60_000, settings.generationConcurrency.video, async () => {
        ...
        const billingRequestId = clean(body.context?.clientRequestId) || clean(request.headers.get("x-venlinks-client-request-id")) || `video-request:${user.id}:${Date.now()}`;
...
export async function createUpstream(
    ...
    for (const path of createPaths) {
        ...
        try {
            ...
            const pointsCost = billedPointsCost(response.headers.get("x-venlinks-points-cost"));
            const pointsRecordId = response.headers.get("x-venlinks-points-record-id") || undefined;
        ...
        return {
            ...
            pointsCost: billedPointsCost(response.headers.get("x-venlinks-points-cost")),
            pointsUnits: videoUnits(raw, multipliers),
            pointsRecordId: response.headers.get("x-venlinks-points-record-id") || undefined,
        };
    }

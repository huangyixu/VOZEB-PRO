...
export async function POST(request: Request, { params }: { params: Promise<{ channelId: string }> }) {
    ...
    try {
        ...
        const signature = request.headers.get("x-venlinks-signature") || request.headers.get("x-signature") || "";
        ...
        const eventId = request.headers.get("x-venlinks-event-id")?.trim() || readProviderString(payload, "event.id", EVENT_KEYS);
